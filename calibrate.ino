...
void calibrateLoadCell() {
  ...
  while (_resume == false) {
    GetUserInput(buf);
    if (buf[0] == 'y') {
      if( file.open(CALIBRATIONS_FILENAME, Adafruit_LittleFS_Namespace::FILE_O_WRITE) )
      {
        // Mark calibration as valid before writing so loadCalibration()
        // accepts the file on next boot (checks for magic byte 0xAB).
        nvram_settings.calibrated = 0xAB;
