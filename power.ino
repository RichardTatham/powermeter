...
// Min pause between BLE updates (millis).
// NOTE: If this value is greater than the time for one crank rotation,
// we will miss revolutions. At 1500ms we reliably catch cadence down to
// ~40 RPM (one rotation every 1500ms). Below that the crank counter stalls,
// which is acceptable — head units typically show 0 cadence when coasting.
...
void loop() {
  ...
  // Cumulative crank revolution counter transmitted to the head unit.
  // uint16_t rolls over naturally at 65535 (~12 hours at 90 RPM), which
  // the head unit handles correctly per the BLE Cycling Power spec.
  static uint16_t totalCrankRevs = 0;

  // Last Crank Event Time in 1/1024-second units (uint16_t).
  // This is captured at the moment a crank revolution is detected and held
  // frozen until the next revolution. The head unit derives cadence from the
  // delta between consecutive values:
  //   cadence (RPM) = 60 * 1024 / (crankEventTime_n - crankEventTime_n-1)
  // uint16_t rollover at 65535 (~64 s) is expected and handled by the head unit.
  ...
  if (Bluefruit.connected()) {
    ...
    // Check to see if updateTime() determines the cranks are spinning (in which
    // case it'll aim to update once per revolution).
    bool pedaling = false;
    if (timeSinceLastUpdate > updateTime(dps, &pedaling) && numPolls > 2) {
      ...
      // Update crank revolution data when a revolution is detected.
      // Capture the event timestamp NOW — at the moment of detection —
      // not later when notify() is called. The head unit requires the
      // timestamp of the actual crank event, not the BLE update time.
      // Converting millis() to 1/1024-second units:
      //   1/1024 s = 0.9765625 ms, so: ticks = millis * 1024 / 1000
      // Cast to uint16_t is intentional — natural rollover at 65535
      // (~64 seconds) is expected and handled correctly by the head unit.
      if (pedaling) {
        totalCrankRevs++;
      ...
      // Publish to BLE. Pass the frozen crank event timestamp — blePublishPower
      // re-sends it unchanged if no new revolution occurred this update, which
      // is correct: the head unit only recalculates cadence when crankRevs
      // increments, and ignores packets where only the event time changes.
